...
public:
    SimpleLRU(size_t max_size = 1024) : _max_size(max_size), _act_size(0), _lru_head(nullptr), _lru_tail(nullptr) {}
...
private:
    ...
    std::size_t _act_size; //actual size
    ...
    bool Erase(lru_node &node);

    bool Erase_ptr(lru_node &node);

    bool Insert_ptr(lru_node &node);
